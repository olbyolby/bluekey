...
3. Install [system daemon](#system-daemon), if desired.

#### Starting the Daemon (Manual)

The daemon can be started by running its command without any arguments, `bluekeyd`. To access `/dev/input devices`, it will need to either be run as root, or, more preferably, as part of the input group, using something like `sudo --preserve-env setpriv --regid $(id -g $USER) --reuid $(id -u $USER) --groups input,$(id -G $USER | sed "s/ /,/g") bash` to use the group temporarily, or a custom user.
...
| Shell/GUI           | Shell, DBus     | Shell                                             | Xorg(GUI)                                                | Shell                                                                              |
